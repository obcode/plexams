...
planToHTMLTable maybeExams plan =
  ...
  unscheduledExamsPlannedByOthers ++ after
  where
    ...
    before =
      ...
      "</head><body>" ++ "<h1>Prüfungsplan " ++ unpack sName ++ "</h1>\n"
    after =
      ...
      "});\n" ++ "</script>" ++ "</body></html>"
    caption =
      "<span class=\"tiptext\">Basisfarbe</span>\n" ++
    ...
    planToHTMLTable' =
      let header = "" : map show (examDays $ semesterConfig plan)
          ...
          showExam exam =
            ...
            ")" ++ "<div class=\"description\">" ++ show exam ++ "</div></div>"
          ...
          mkConflicts ancode =
            concatMap (M.keys . M.findWithDefault M.empty ancode . olOverlaps) $
    ...
    unscheduledExamsPlannedByOthers =
      insideTag "ul" $ concatMap (insideTag "li" . show) plannedByOtherExams
