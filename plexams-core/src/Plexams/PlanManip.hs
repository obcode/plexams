...
addRegistrationsToExam registrations' exam =
  ...
  where
    regForExam = M.lookup (anCode exam) (regs registrations')
    (_, otherGroups) = partition ((== regGroup) . groupDegree) $ groups exam
    addRegToExam exam' regSum =
      exam'
      {groups = Group regGroup Nothing Nothing (Just regSum) : otherGroups}
    regGroup = read $ regsGroup registrations'
    examNotForGroup =
      notElem (regsGroup registrations') $
      map (show . groupDegree) $ groups exam
