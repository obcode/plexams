...
makePlan exams'' semesterConfig' pers constraints' = foldr
 ...
 where
  ...
  (examsPlannedNotByMe', unscheduledExams') = partition
...
updateExamByAncodeWith plan ancode f
  | isScheduledAncode ancode plan = updateScheduledExamByAncodeWith plan
                                                                    ancode
                                                                    f
  | isUnscheduledAncode ancode plan = updateUnscheduledExamByAncodeWith plan
                                                                        ancode
                                                                        f
  | otherwise = plan
...
updateScheduledExamByAncodeWith plan ancode f =
  let updatedExamsInSlot =
        M.alter (fmap f) ancode $ examsInSlot oldSlotContents
