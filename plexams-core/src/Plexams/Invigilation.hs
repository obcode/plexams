...
sumPercentInvigilator invigilator' =
  let percent = 100 * invigilatorPartTime invigilator'
      overthis =
        if invigilatorOvertimeThisSemester invigilator' == 1.5
          then 0.5
          else 0
      overlast =
        if invigilatorOvertimeLastSemester invigilator' == 1.5
          then 0.5
          else 0
...
invigilatorsWithMinutesPlanned plan =
  let reserves :: [(PersonID, Integer)]
      reserves =
        mapMaybe
          (fmap
             (\invigilator' -> (invigilatorID invigilator', minutesForReserve)) .
           reserveInvigilator) $
        M.elems $ slots plan
...
invigilationsPerPerson plan =
  let invigilatorsPlanned' = invigilatorsPlanned plan
      ...
      mkReserveInvigilations invigilatorID' =
        ...
        filter
          ((== Just invigilatorID') .
           fmap invigilatorID . reserveInvigilator . snd) $
        M.toList $ slots plan
      mkExamInvigilations invigilatorID' =
        map
          (\((d, s), ((roomID', duration'), _)) ->
             Invigilation
             { invigilationInvigilatorID = invigilatorID'
             , invigilationDay = d
             , invigilationSlot = s
