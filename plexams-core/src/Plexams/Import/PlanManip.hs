...
data ImportAddInvigilator = ImportAddInvigilator Integer [Int] (Maybe String)
  deriving Show

instance Y.FromJSON ImportAddInvigilator where
    parseJSON (Y.Object v ) = ImportAddInvigilator
                         <$> v Y..: "id"
                         <*> v Y..: "slot"
                         <*> v Y..:? "room"
    parseJSON _          = empty
...
importAddInvigilatorToRoomOrSlotFromYAMLFile :: FilePath
  -> IO (Maybe [AddInvigilatorToRoomOrSlot])
importAddInvigilatorToRoomOrSlotFromYAMLFile =
