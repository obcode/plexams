...
checkFixedSlot fixedSlots exam =
  if ([] == entry')
    then Ok
      ...
      then Ok
      else FixedSlotError $ "Exam is bound to the fixed Slot: (Day: " ++ (show $ planManipDay entry)
                          ++ " Slot: " ++ (show $ planManipSlot entry) ++ ")."
  where
    entry = AddExamToSlot {planManipAnCode = fst $ head entry'
...
checkOnOneOfTheseDays onOneOfTheseDays' exam =
  if (length entry' == 0)
    then Ok
      ...
      then Ok
      else OnOneOfTheseDaysError $ "Exam is bound to the following days: " ++ (show $ snd $ head entry')

  where
    allowed = elem (toInteger $ planManipDay exam) (map toInteger $ snd $ head entry')
...
checkNotOnSameDay notOnSameDay' plan exam =
  if (entries == [])
    then Ok
      ...
      then Ok
      else NoOnSameDayError $ "Exam must not be on the same day as : " ++ (show entries)
