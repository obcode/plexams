function viewDetails(event, anCode) {
  ...
  if (event.currentTarget.className.includes("div_select")) {
    ...
    $.getJSON(endpoints.exams, function(exams) {
      ...
      output += ` <br>
                  <a href="mailto:${exam.lecturer.personEmail}"> ${
        exam.lecturer.personID
      }.
                  <span class="lecturer">${
      ...
      if (exam.handicapStudents.length > 0) {
        output +=
          '<span class="NTA">Nachteilsausgleich</span><ul class="handicapStudents">';
...
function toggleSelect(thisObj) {
  ...
  thisObj
    .parents("div")
    .find("div")
    .removeClass("conflictsLT5");
  thisObj
    .parents("div")
    .find("div")
    .removeClass("conflictsLT10");
  thisObj
    .parents("div")
    .find("div")
    .removeClass("conflictsLT20");
  thisObj
    .parents("div")
    .find("div")
    .removeClass("conflicts");
  thisObj
    .parents("div")
    .find("div")
    .removeClass("conflictingSlot");
  thisObj
    .parents("div")
    .find("div")
    .removeClass("examsBySameLecturer");
...
function allowDropExam(ev) {
  if (
    ev.currentTarget.classList.contains("outer") ||
    ev.currentTarget.className === "outerUnscheduled"
  ) {
    ev.preventDefault();
  }
}
