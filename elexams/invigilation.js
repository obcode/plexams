    ...
    $.ajax({
      ...
      success: slots => {
        $.getJSON(endpoints.slotsPerDay, function(slotsPerDay) {
          let modalOutput = "";
          let output = `<table class="invigilators">
              <tr>
                <td>
            ...
            if (examData.length > 0) {
              ...
              for (let j in examData) {
                ...
                for (let k in exam.rooms) {
                  ...
                  output += `" onclick="openModal('modal-${exam.anCode}-${
                    room.roomID
                  }-${room.studentsInRoom.length}')"
                                ondrop="dropInvigilator(event, ${exam.slot}, '${
                    room.roomID
                  }')" ondragover="allowDrop(event)">
                                ${exam.anCode}. ${exam.name}<br>
                                Prüfer: ${exam.lecturer.personShortName}<br>
                                <span class="room-${room.roomID}">${
                    room.roomID
                  }</span>
                                (${room.studentsInRoom.length}/${
                    room.maxSeats
                  ...
                  output += `</div></td>`;
                  modalOutput += `<div id="modal-${exam.anCode}-${
                    room.roomID
                  }-${room.studentsInRoom.length}" class="modal">
                                    <div class="modal-content">
                                      <span id="modal-${exam.anCode}-${
                    room.roomID
                  }-${room.studentsInRoom.length}-close">&times;</span>
                                      <h2>${room.roomID}: ${exam.anCode}. ${
                    exam.name
                  ...
                  modalOutput += `</h2>`;
                  if (room.invigilator !== null) {
                    modalOutput += `<h3>Aufsicht: ${
                      room.invigilator.invigilatorID
                    }. ${room.invigilator.invigilatorName}
                      <span id="remove-${exam.anCode}-${room.roomID}-${
                      room.invigilator.invigilatorID
                    }">
                      entfernen</span>
                      </h3>`;
                  ...
                  for (let s in room.studentsInRoom) {
                    ...
                    modalOutput += `<li> ${student.studentFamilyname}, ${
          ...
          success: invigs => {
            for (let i in invigs) {
              ...
              for (let j in invigilators) {
                ...
                output += `"
                          draggable="true" ondragstart="dragInvigilatorStart(event, ${
                            invig.invigilatorID
                          })">
                          <span class="invigName">${
                            invig.invigilatorName
                          }</span> (${invig.invigilatorID})<br>offen:`;
                if (
                  invig.invigilatorMinutesTodo -
                    invig.invigilatorsMinutesPlanned <
                  0
                ) {
                ...
                output += `${invig.invigilatorMinutesTodo -
                  invig.invigilatorsMinutesPlanned} Minuten</span><br>
                          geplante Tage: ${
                            invig.invigilatorInvigilationDays
                          }<br>
...
const dropInvigilator = (event, dayIdx, slotIdx, room) => {
  ...
  const dropped = addInvigilatorToExamOrSlot(
    invigilatorID,
    dayIdx,
    slotIdx,
    room
...
let fetchInvigilators = () => {
  $.getJSON(endpoints.invigilators, invigilators => {
    let output = `<ul class="invigilations">
        <li class="invigilations">Summe Aufsichten Prüfungen: ${
          invigilators[0].invigilationsSumExamRooms
        } Minuten</li>
        <li class="invigilations">Summe Reserveaufsichten: ${
          invigilators[0].invigilationsSumReserve
        } Minuten</li>
        <li class="invigilations">Summe Beisitz mündliche Prüfungen: ${
          invigilators[0].invigilationsSumOralExams
        } Minuten</li>
        <li class="invigilations">Summe Mastergespräche: ${
          invigilators[0].invigilationsSumMaster
        } Minuten</li>
        <li class="invigilations">Summe Aufsichten Livecoding: ${
          invigilators[0].invigilationsSumLivecoding
        } Minuten</li>
        </ul>
      ...
      <tr class="invigilatorList">
        ...
        <th class="invigilatorList">Anforderungen</th>
    ...
    for (let i in invigilators[1]) {
      ...
      output += `<tr class="invigilatorList">
             ...
             <td class="invigilatorList">`;
      if (!invigilator.invigilatorHasConstraints) {
        output += "keine";
      }
      output += `</td>
             ...
             <td class="invigilatorList">${
               invigilator.invigilatorExcludedDays
             }</td>
             <td class="invigilatorList invigilatorListContent${
               invigilator.invigilatorInvigilationDays
             }">${invigilator.invigilatorInvigilationDays}</td>
             <td class="invigilatorList invigilatorListContent${
               invigilator.invigilatorMinutesTodo
             }">${invigilator.invigilatorMinutesTodo}</td>
             <td class="invigilatorList invigilatorListContent${
               invigilator.invigilatorsMinutesPlanned
             }">${invigilator.invigilatorsMinutesPlanned}</td>`;
      if (
        invigilator.invigilatorMinutesTodo -
          invigilator.invigilatorsMinutesPlanned <
        0
      ...
      output += `${invigilator.invigilatorMinutesTodo -
        invigilator.invigilatorsMinutesPlanned}</td>
             <td class="invigilatorList invigilatorListContentPartime${
               invigilator.invigilatorPartTime
             }">${invigilator.invigilatorPartTime}</td>
             <td class="invigilatorList invigilatorListContent${
               invigilator.invigilatorOralExams
             }">${invigilator.invigilatorOralExams}</td>
             <td class="invigilatorList invigilatorListContent${
               invigilator.invigilatorMaster
             }">${invigilator.invigilatorMaster}</td>
             <td class="invigilatorList invigilatorListContent${
               invigilator.invigilatorLiveCoding
             }">${invigilator.invigilatorLiveCoding}</td>
             <td class="invigilatorList invigilatorListContent${
               invigilator.invigilatorOvertimeLastSemester
             }">${invigilator.invigilatorOvertimeLastSemester}</td>
             <td class="invigilatorList invigilatorListContent${
               invigilator.invigilatorOvertimeThisSemester
             }">${invigilator.invigilatorOvertimeThisSemester}</td>
             <td class="invigilatorList invigilatorListContent${
               invigilator.invigilatorFreeSemester
             }">${invigilator.invigilatorFreeSemester}</td>
           </tr>`;
    }
    output += `</tbody>
              </table>`;
