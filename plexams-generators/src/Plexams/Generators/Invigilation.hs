...
generateInvigilationsForOwnExams :: Plan
                                 -> Writer [AddInvigilatorToRoomOrSlot] Plan
generateInvigilationsForOwnExams plan =
  foldM generateInvigilationsForOwnExams'
        plan
        $ M.keys $ invigilators plan
...
generateInvigilationsForOwnExams' :: Plan -> PersonID
                                 -> Writer [AddInvigilatorToRoomOrSlot] Plan
generateInvigilationsForOwnExams' plan invigilatorID' = do
  let invigilatorsExam = filter (any isNothing . map invigilator . rooms)
                       $ filter ((==1)  . length . rooms)
                       $ filter ((==invigilatorID') . personID . lecturer)
                       $ scheduledExams plan
      updateExamByAncodeWith' :: Plan -> Exam
                              -> Writer [AddInvigilatorToRoomOrSlot] Plan
      updateExamByAncodeWith' plan' exam = do
        let slot' = fromJust $ slot exam
            maybeInvigilator = M.lookup invigilatorID'
                                   $ invigilators plan'
        case maybeInvigilator of
          ...
          Just invigilator' ->
            ...
            then do
              tell [AddInvigilatorToRoomOrSlot
                      invigilatorID'
                      slot'
                      (Just $ roomID $ head $ rooms exam)
                   ]
              let plan'' = updateExamByAncodeWith plan' (anCode exam)
                        (\e -> e { rooms = map (\r -> r {
                                invigilator = maybeInvigilator
                              }
                            ) $ rooms e
                          }
                        )
                  plan''' = addMinutesPlannedToInvigilator plan'' invigilatorID'
